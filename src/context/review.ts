...
const applyImportContext = (file: PRFile): importedFunctions[] => {
  ...
  patches.forEach((patch) => {
    patch.hunks.forEach((hunk) => {
      ...
      //find the function calls in the changed lines
      ...
      //find all imported files and functions
      if (file.current_contents) {
        ...
        //Identify external functions and push them
        repo_imports.forEach((import_statement) => {
          externalFunctions.push(findExternalFunction(import_statement));
        });
      } else {
...
const findExternalFunction = (import_statement: string): importedFunctions => {
  const result: importedFunctions = {
    filepath: "",
    filename: "",
    functions: new Set<string>(),
  };
  if (import_statement.includes("from")) {
    const [importPart, pathPart] = import_statement
      .split("from")
      .map((s) => s.trim());
    ...
    if (importPart.includes("{")) {
      const matches = importPart.match(/{([^}]+)}/);
      if (matches && matches[1]) {
        matches[1].split(",").forEach((func) => {
          const cleanFunc = func.trim().split(" as ")[0];
          result.functions.add(cleanFunc);
        });
      }
    } else {
      const defaultImport = importPart.replace("import", "").trim();
      if (defaultImport) {
        result.functions.add(defaultImport);
      }
    }
  } else if (import_statement.includes("*")) {
    // Format: import * as name from './path/to/file'
    const matches = import_statement.match(
      /\* as (\w+) from ['"](.[^'"]+)['"]/
    );
    if (matches) {
      const [_, namespace, filepath] = matches;
      result.filepath = filepath;
      result.filename = filepath.split("/").pop() || "";
      result.functions.add(namespace);
    }
  }
  return result;
};
